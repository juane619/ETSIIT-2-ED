...
*/
 ...
 /** @brief Constructor por defecto de la clase mutaci�n
 */
mutacion::mutacion(){
   this->ID="";
   this->chr="";
...
/**
  @brief Constructor copia, recibe una mutaci�n y se copia de esta
...
mutacion::mutacion(const string& str){
   int pos_i=0, pos_f=0, pos_f2=0, aux_int=0, i=0;
   ...
   //FALTA A�ADIR AL VECTOR DE STRING CORRECTAMENTE
   if(!aux_str.empty()){
        pos_f= aux_str.find('|', 0);
        pos_f2= aux_str.find(',', 0);

        if(pos_f != -1 || pos_f2 != -1){
            if(pos_f == -1)
                pos_f=pos_f2;
            else if(pos_f2!=-1)
                pos_f=pos_f<pos_f2?pos_f:pos_f2;
            
           pos_i=0;
           do{
              aux_str2= aux_str.substr(pos_i, pos_f);
              pos_i= pos_f+1;
              this->genes.push_back(aux_str2);
              pos_f= aux_str.find('|', pos_i);
                pos_f2= aux_str.find(',', pos_i);
                if(pos_f == -1)
                    pos_f=pos_f2;
                else if(pos_f2!=-1)
                    pos_f=pos_f<pos_f2?pos_f:pos_f2;
   ...
   if(!aux_str.empty()){
      pos_f= aux_str.find(',', 0);  //Obtendr� siempre una separacion por coma
...
pos_i= str.find("COMMON=");
   ...
   //Tenemos ya el n� de enfermedades con sus nombres en el vector, falta a�adir sus ID y sus bases de datos.///
...
ostream& operator<< (ostream& os, const mutacion& m){
   ...
   os << "N� cromosoma: " << m.getChr() <<endl <<   //imprimir cromosoma
...
@param[in]  m objeto mutaci�n
  @return true en caso de ser menor a la pasada, false en caso contrario
	*/
bool mutacion::operator<(const mutacion & m) const{
  ///El orden viene determinado por Chr y pos. El primer criterio es el numero de cromosoma. El orden para el n�mero de cromosoma se rige por "1"<"2"<"3"<->->-><"22"<"X"<"Y"<"MT". Dos mutaciones del mismo cromosoma deben ordenarse seg�n su posici�n, de menor posici�n a mayor (orden natural de enteros)->
  
    if(getChr()=="MT"){            //en caso de que sea MT, distinguimos dos casos: 
      if(m.getChr()!="MT")              //que el parametro no sea MT
         return false;
  }
  else if(getChr()>="X"){             //en caso de que sea X, Y
     if(m.getChr()=="MT")               //si el parametro es MT
        return true;
     else if(m.getChr()>="X"){         //si el parametro es X,Y
         if(getChr() != m.getChr())     //si no son iguales, se comprueba cual es cada uno
            return getChr()<m.getChr();
     }
  }
  else{                       //en caso de que sea numero
      if(m.getChr()>="MT"){      //si el parametro es X, Y, MT
         return true;
      }
      else{                      //si el parametro es numero, igual que el this
            //cout << m.getChr() << "\n";
