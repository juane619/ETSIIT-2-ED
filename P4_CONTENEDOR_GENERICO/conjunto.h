...
public:
	...
	*/
	pair<typename conjunto<T,CMP>::iterator,bool> insert (const value_type& val);
	pair<typename conjunto<T,CMP>::iterator,bool> insert (value_type& val);
...
private:
	vector<value_type> vdatos; // vector que almacena los elementos del conjunto
